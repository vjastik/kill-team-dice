...
export const SpecialRulesPicker: React.FC<Props> = props => {
  ...
  const {attack: {specialRules}, changeAttackSettings} = React.useContext(DataContext)

  const handleChoose = (
    _event: React.SyntheticEvent,
    data: AutocompleteValue<ShootSpType, false, false, false>
  ) => {
    if (!data) return
    if (specialRules.some(item => item.value === data.value)) return;

    changeAttackSettings('specialRules', [...specialRules, {
      label: data.label,
      value: data.value,
      ...(data.withX ? { xValue: 1 } : {})
    }])
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Autocomplete
          placeholder="Select special rule"
          options={SHOOT_SP}
        ...
        <Box sx={{display: 'grid', gap: 2, paddingTop: 2}}>
          {specialRules.map((item, index) => {
            return (
              <SpecialRuleItem key={`sp-${item.value}`} data={item} index={index}/>
            )
          })}
        </Box>
      </Sheet>
...
const SpecialRuleItem: React.FC<ItemProps> = props => {
  const { data: {label, value, xValue}, index} = props
  const { changeAttackSettings, attack: {specialRules} } = React.useContext(DataContext)
  ...
  const handleRemove = () => {
    changeAttackSettings('specialRules', specialRules.filter(item => item.value !== value))
  }
  ...
  return (
    <Box sx={{
      ...
      <Button size="sm" color="neutral" onClick={handleRemove}>-</Button>
      <Typography level="body-sm">{label}</Typography>
      {!spFromList?.withX ? null : (
        <>
          <Button size="sm" onClick={handleChangeXValue(-1)}>-1</Button>
          <Typography textAlign="center">{xValue}{plus}</Typography>
          <Button size="sm" onClick={handleChangeXValue(1)}>+1</Button>
        </>
      )}
    </Box>
